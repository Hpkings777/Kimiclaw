...
public class CycleDetectingLockFactory {
    ...
    private static final ThreadLocal<List<LockGraphNode>> acquiredLocks = new ThreadLocal<List<LockGraphNode>>() { // from class: com.google.common.util.concurrent.CycleDetectingLockFactory.1
        @Override // java.lang.ThreadLocal
        public List<LockGraphNode> initialValue() {
    ...
    public void aboutToAcquire(CycleDetectingLock cycleDetectingLock) {
        ...
        List<LockGraphNode> list = acquiredLocks.get();
        Objects.requireNonNull(list);
        List<LockGraphNode> list2 = list;
        LockGraphNode lockGraphNode = cycleDetectingLock.getLockGraphNode();
        lockGraphNode.checkAcquiredLocks(this.policy, list2);
        list2.add(lockGraphNode);
    }
    ...
    public static void lockStateChanged(CycleDetectingLock cycleDetectingLock) {
        ...
        for (int size = list2.size() - 1; size >= 0; size--) {
            if (list2.get(size) == lockGraphNode) {
                list2.remove(size);
                return;
            }
        }
