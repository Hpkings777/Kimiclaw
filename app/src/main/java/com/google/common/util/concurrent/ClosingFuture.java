...
public final class ClosingFuture<V> {
    ...
    public static class Combiner {
        ...
        private ImmutableList<FluentFuture<?>> inputFutures() {
            return FluentIterable.from(this.inputs).transform(new C2768ji(1)).toList();
        }
    ...
    private <X extends Throwable, W extends V> ClosingFuture<V> catchingMoreGeneric(Class<X> cls, final ClosingFunction<? super X, W> closingFunction, Executor executor) {
        ...
        return (ClosingFuture<V>) derive(this.future.catchingAsync(cls, new AsyncFunction<X, W>(this) { // from class: com.google.common.util.concurrent.ClosingFuture.6
            final /* synthetic */ ClosingFuture this$0;
    ...
    public static void closeQuietly(AutoCloseable autoCloseable, Executor executor) {
        ...
        try {
            executor.execute(new J1(autoCloseable, 15));
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$finishToFuture$0() {
        State state = State.WILL_CLOSE;
        State state2 = State.CLOSING;
        checkAndUpdateState(state, state2);
        close();
        checkAndUpdateState(state2, State.CLOSED);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$finishToValueAndCloser$0(ValueAndCloserConsumer valueAndCloserConsumer) {
        provideValueAndCloser(valueAndCloserConsumer, this);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static /* synthetic */ ClosingFuture lambda$withoutCloser$0(AsyncFunction asyncFunction, DeferredCloser deferredCloser, Object obj) throws Exception {
        return from(asyncFunction.apply(obj));
    }
    ...
    public static <V, U> AsyncClosingFunction<V, U> withoutCloser(AsyncFunction<V, U> asyncFunction) {
        ...
        return new C2113f2(asyncFunction, 3);
    }
    ...
    public FluentFuture<V> finishToFuture() {
        if (compareAndUpdateState(State.OPEN, State.WILL_CLOSE)) {
            ...
            this.future.addListener(new J1(this, 16), MoreExecutors.directExecutor());
        } else {
    ...
    public void finishToValueAndCloser(ValueAndCloserConsumer<? super V> valueAndCloserConsumer, Executor executor) {
        ...
        if (compareAndUpdateState(State.OPEN, State.WILL_CREATE_VALUE_AND_CLOSER)) {
            this.future.addListener(new O4(10, this, valueAndCloserConsumer), executor);
            return;
        }
        int iOrdinal = this.state.get().ordinal();
        if (iOrdinal == 1) {
