...
public final class ClassPath {
            ...
            for (File file2 : fileArrListFiles) {
                ...
                if (file2.isDirectory()) {
                    ...
                    if (set.add(canonicalFile)) {
                        scanDirectory(canonicalFile, AbstractC4671xI0.j(str, name, "/"), set, builder);
                        set.remove(canonicalFile);
                    }
                } else {
                    String strI = AbstractC4671xI0.i(str, name);
                    if (!strI.equals("META-INF/MANIFEST.MF")) {
                        builder.add(ResourceInfo.of(file2, strI, this.classloader));
                    }
                }
            }
        }
    ...
    public ImmutableSet<ClassInfo> getTopLevelClasses() {
        return FluentIterable.from(this.resources).filter(ClassInfo.class).filter(new C0312Fm(0)).toSet();
    }
