...
public class MinimalECIInput implements ECIInput {
    ...
    public static int[] encodeMinimally(String str, ECIEncoderSet eCIEncoderSet, int i) {
        ...
        if (i5 < 0) {
            throw new IllegalStateException(AbstractC4671xI0.z("Failed to encode \"", str, "\""));
        }
        ArrayList arrayList = new ArrayList();
        for (InputEdge inputEdge3 = inputEdgeArr[length][i5]; inputEdge3 != null; inputEdge3 = inputEdge3.previous) {
            if (inputEdge3.isFNC1()) {
                arrayList.add(0, Integer.valueOf(IMAPStore.RESPONSE));
            } else {
    ...
    public char charAt(int i) {
        if (i < 0 || i >= length()) {
            throw new IndexOutOfBoundsException(YI0.h(i, ""));
        }
        if (isECI(i)) {
            throw new IllegalArgumentException(AbstractC2039eV.i(i, "value at ", " is not a character but an ECI"));
        }
    ...
    public int getECIValue(int i) {
        ...
        throw new IllegalArgumentException(AbstractC2039eV.i(i, "value at ", " is not an ECI but a character"));
    }
