...
class ObjectCountHashMap<K> {
    ...
    public final class MapEntry extends Multisets.AbstractEntry<K> {
        ...
        public void updateLastKnownIndex() {
            int i = this.lastKnownIndex;
            if (i == -1 || i >= ObjectCountHashMap.this.size() || !Objects.equals(this.key, ObjectCountHashMap.this.keys[this.lastKnownIndex])) {
    ...
    private void resizeMeMaybe(int i) {
        ...
        if (i > length) {
            ...
            if (iMax < 0) {
                iMax = IntCompanionObject.MAX_VALUE;
            }
            if (iMax != length) {
                resizeEntries(iMax);
            }
    ...
    private static long swapNext(long j, int i) {
        return (j & HASH_MASK) | (4294967295L & ((long) i));
    }
    ...
    public int indexOf(Object obj) {
        ...
        while (next != -1) {
            ...
            if (getHash(j) == iSmearedHash && Objects.equals(obj, this.keys[next])) {
                return next;
            }
            next = getNext(j);
        }
    ...
    public int put(@ParametricNullness K k, int i) {
        ...
        if (i3 == -1) {
            ...
            while (true) {
                long j = jArr[i3];
                if (getHash(j) == iSmearedHash && Objects.equals(k, objArr[i3])) {
                    int i4 = iArr[i3];
                    iArr[i3] = i;
                    return i4;
                }
    ...
    private int remove(Object obj, int i) {
        ...
        int i3 = -1;
        while (true) {
            if (getHash(this.entries[i2]) == i && Objects.equals(obj, this.keys[i2])) {
