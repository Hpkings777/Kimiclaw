...
public final class IntMath {
    ...
    @VisibleForTesting
    static final int[] powersOf10 = {1, 10, 100, IMAPStore.RESPONSE, 10000, 100000, 1000000, 10000000, 100000000, 1000000000};

    @VisibleForTesting
    static final int[] halfPowersOf10 = {3, 31, 316, 3162, 31622, 316227, 3162277, 31622776, 316227766, IntCompanionObject.MAX_VALUE};
    private static final int[] factorials = {1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600};

    @VisibleForTesting
    static final int[] biggestBinomials = {IntCompanionObject.MAX_VALUE, IntCompanionObject.MAX_VALUE, 65536, 2345, 477, 193, 110, 75, 58, 49, 43, 39, 37, 35, 34, 34, 33};
    ...
    public static int ceilingPowerOfTwo(int i) {
        ...
        throw new ArithmeticException(AbstractC2039eV.i(i, "ceilingPowerOfTwo(", ") not representable as an int"));
    }
    ...
    public static int checkedPow(int i, int i2) {
        ...
        while (i2 != 0) {
            if (i2 == 1) {
                return Math.multiplyExact(iMultiplyExact, i);
            }
            if ((i2 & 1) != 0) {
                iMultiplyExact = Math.multiplyExact(iMultiplyExact, i);
            }
            i2 >>= 1;
            if (i2 > 0) {
    ...
    public static int factorial(int i) {
        MathPreconditions.checkNonNegative("n", i);
        int[] iArr = factorials;
        return i < iArr.length ? iArr[i] : IntCompanionObject.MAX_VALUE;
    }
    ...
    public static int gcd(int i, int i2) {
        MathPreconditions.checkNonNegative(IEncryptorType.DEFAULT_ENCRYPTOR, i);
    ...
    public static int mod(int i, int i2) {
        ...
        throw new ArithmeticException(AbstractC2039eV.i(i2, "Modulus ", " must be > 0"));
    }
    ...
    public static int saturatedAbs(int i) {
        return i == Integer.MIN_VALUE ? IntCompanionObject.MAX_VALUE : Math.abs(i);
    }
    ...
    public static int saturatedPow(int i, int i2) {
        MathPreconditions.checkNonNegative("exponent", i2);
        if (i == -2) {
            return i2 >= 32 ? (i2 & 1) + IntCompanionObject.MAX_VALUE : (i2 & 1) == 0 ? 1 << i2 : (-1) << i2;
        }
        if (i == -1) {
            return (i2 & 1) == 0 ? 1 : -1;
        }
        ...
        if (i == 2) {
            return i2 >= 31 ? IntCompanionObject.MAX_VALUE : 1 << i2;
        }
        int i3 = ((i >>> 31) & i2 & 1) + IntCompanionObject.MAX_VALUE;
