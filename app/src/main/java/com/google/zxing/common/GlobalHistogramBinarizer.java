    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (int i5 = 0; i5 < height; i5++) {
            ...
            for (int i7 = 0; i7 < width; i7++) {
                if ((matrix[i6 + i7] & UnsignedBytes.MAX_VALUE) < iEstimateBlackPoint) {
                    bitMatrix.set(i7, i5);
                }
            }
        }
    ...
    public BitArray getBlackRow(int i, BitArray bitArray) throws NotFoundException {
        ...
        if (width < 3) {
            for (int i4 = 0; i4 < width; i4++) {
                if ((row[i4] & UnsignedBytes.MAX_VALUE) < iEstimateBlackPoint) {
                    bitArray.set(i4);
                }
            }
        } else {
            int i5 = row[0] & UnsignedBytes.MAX_VALUE;
            int i6 = row[1] & UnsignedBytes.MAX_VALUE;
            int i7 = 1;
            while (i7 < width - 1) {
                int i8 = i7 + 1;
                int i9 = row[i8] & UnsignedBytes.MAX_VALUE;
                if ((((i6 * 4) - i5) - i9) / 2 < iEstimateBlackPoint) {
                    bitArray.set(i7);
                }
                i5 = i6;
                i7 = i8;
