...
public final class TreeRangeMap<K extends Comparable, V> implements RangeMap<K, V> {
    ...
    public static final class RangeMapEntry<K extends Comparable, V> extends AbstractMap.SimpleImmutableEntry<Range<K>, V> {
        ...
        public boolean contains(K k) {
            return ((Range) getKey()).contains(k);
        }

        public Cut<K> getLowerBound() {
            return (Cut<K>) ((Range) getKey()).lowerBound;
        }

        public Cut<K> getUpperBound() {
            return (Cut<K>) ((Range) getKey()).upperBound;
        }
    ...
    private static <K extends Comparable, V> Range<K> coalesce(Range<K> range, V v, Map.Entry<Cut<K>, RangeMapEntry<K, V>> entry) {
        return (entry != null && ((Range) entry.getValue().getKey()).isConnected(range) && entry.getValue().getValue().equals(v)) ? (Range<K>) range.span((Range) entry.getValue().getKey()) : range;
    }
    ...
    public Range<K> span() {
        ...
        return Range.create(((Range) entryFirstEntry.getValue().getKey()).lowerBound, ((Range) entryLastEntry.getValue().getKey()).upperBound);
    }
