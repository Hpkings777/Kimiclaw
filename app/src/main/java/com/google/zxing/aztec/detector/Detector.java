...
public final class Detector {
    ...
    public static final class CorrectedParameter {
        ...
        public CorrectedParameter(int i, int i2) {
            this.data = i;
    ...
    public static final class Point {
        ...
        public String toString() {
            ...
            sb.append(this.x);
            sb.append(' ');
            return AbstractC4671xI0.m(sb, this.y, '>');
        }
    }
    ...
    private int extractParameters(ResultPoint[] resultPointArr) throws NotFoundException {
        ...
        CorrectedParameter correctedParameterData = getCorrectedParameterData(j3, this.compact);
    ...
    private ResultPoint[] getBullsEyeCorners(Point point) throws NotFoundException {
        ...
        this.compact = i == 5;
        return expandSquare(new ResultPoint[]{new ResultPoint(point.getX() + 0.5f, point.getY() - 0.5f), new ResultPoint(point2.getX() + 0.5f, point2.getY() + 0.5f), new ResultPoint(point3.getX() - 0.5f, point3.getY() + 0.5f), new ResultPoint(point4.getX() - 0.5f, point4.getY() - 0.5f)}, (r0 * 2) - 3, this.nbCenterLayers * 2);
    }
    ...
    private int getColor(Point point, Point point2) {
        ...
        int iFloor = (int) Math.floor(fDistance);
        int i = 0;
    ...
    private static CorrectedParameter getCorrectedParameterData(long j, boolean z) throws NotFoundException {
        int i;
        int i2;
        if (z) {
            i = 7;
        ...
        try {
            int iDecodeWithECCount = new ReedSolomonDecoder(GenericGF.AZTEC_PARAM).decodeWithECCount(iArr, i3);
            int i5 = 0;
            for (int i6 = 0; i6 < i2; i6++) {
                i5 = (i5 << 4) + iArr[i6];
            }
            return new CorrectedParameter(i5, iDecodeWithECCount);
    ...
    public AztecDetectorResult detect(boolean z) throws NotFoundException {
        ...
        int iExtractParameters = extractParameters(bullsEyeCorners);
        BitMatrix bitMatrix = this.image;
        int i = this.shift;
        return new AztecDetectorResult(sampleGrid(bitMatrix, bullsEyeCorners[i % 4], bullsEyeCorners[(i + 1) % 4], bullsEyeCorners[(i + 2) % 4], bullsEyeCorners[(i + 3) % 4]), getMatrixCornerPoints(bullsEyeCorners), this.compact, this.nbDataBlocks, this.nbLayers, iExtractParameters);
    }
}
